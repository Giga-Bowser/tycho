use std::{borrow::Borrow, fmt::Display, time::Duration};

#[derive(Clone, Debug, Default)]
pub struct ScopedMap<'a, K, V> {
    pub parent_scopes: Option<&'a Self>,
    pub current_scope: Vec<(K, V)>,
}

impl<'a, K: Eq, V> ScopedMap<'a, K, V> {
    pub fn new_with_parent(parent_scopes: &'a Self) -> Self {
        Self {
    ...
    pub fn get<Q: ?Sized + Eq>(&'a self, key: &Q) -> Option<&'a V>
    where
        K: Borrow<Q>,
    {
        self.current_scope
